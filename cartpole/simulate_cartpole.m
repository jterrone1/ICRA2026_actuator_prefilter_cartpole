function simulate_cartpole()
    ...
    left_loc = 0; % -100 turns off wall
    right_loc = 0; % 100 turns of wall
    ...
    rng(rng_val)
    z0 = [0.2*(rand(1)-0.5); 0; 2*(rand(1)-0.5); 2*(rand(1)-0.5); (rand(1)-0.5)];
    ...
    for i=1:num_steps-1
        ...
        dz = dynamics(z_out(:,i), p, 2, koop_model);
    ...
    %myVideo = VideoWriter(sprintf('Cart-Pole_KLQR_T_%.0f_Trial_%.0f', [tau, rng_val])); 
    ...
    cart = rectangle('Position',[0 0 cart_w cart_h], ...
                     'FaceColor',colors(tau-1,:),'EdgeColor','k','LineWidth',4);
    ...
    for i=1:num_steps
        ...
        set(h_title,'String',  sprintf('KLQR (T=%.0f) Trial %.0f, t=%.2f',[tau rng_val t]) ); % update title
