...
tau = 1;
left_loc = -0.1; % -100 turns off wall
right_loc = 0.1; % 100 turns of wall
...
X_t = load('X_t_500000_cartpole_1_passive.csv');
X_tp1 = load('X_tp1_500000_cartpole_1_passive.csv'); % assuming passive data
...
for i = 1:length(n_values)
    ...
    for j = 1:length(eps_values)
        ...
        weighted_mean = [20*mean_vals(1), 50*mean_vals(2), 0*mean_vals(3), 0*mean_vals(4)];
...
save(fullfile('koopman_models', ...
    ['obs_' num2str(length(koop_model.A)-5) '_eps_' num2str(koop_model.epsilon) '.mat']), 'koop_model');