function simulate_cartpole()
    ...
    modelPath = fullfile(baseDir, 'koopman_models', 'tau_2.0_obs_9_eps_1.1_-0.2_wall_0.2_2_0.05.mat');
    ...
    figure(90)
    e = eig(A);
    e2 = eig(A(6:end, 6:end));
    e3 = eig(A(5, 5));

    fplot(@(t) sin(t), @(t) cos(t), "black", 'LineWidth',1);
    hold on
    scatter(real(e), imag(e), 75, colors(1, :), "filled")
    scatter(real(e2), imag(e2), 75, colors(5, :), "filled")
    scatter(real(e3), imag(e3), 75, colors(3, :), "filled")
    axis equal
    title("Eigenvalues for T = "+1)
    xlabel("Real")
    ylabel("Imag")
    grid on
    xlim([0.8 1.1])
    ylim([-0.2 0.2])
    %scatter(a, 0, 200, "red", "o")
    %}
    ...
    %writematrix(z_out,"cck_lqr_rng5.csv") 
    ...
    %myVideo = VideoWriter(sprintf('Cart-Pole_KLQR_walls_T_%.0f_Trial_%.0f', [tau, rng_val])); 
    myVideo.FrameRate = 1;
    open(myVideo)
    ...
    for i=1:num_steps
        ...
        set(h_title,'String',  sprintf('CCK + LQR (T=%.0f), t=%.2f',[tau, t]) ); % update title
